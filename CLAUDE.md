...
Claude Code のハーネス（CLAUDE.md / settings.json / .mcp.json / hooks / skills / memory / MCP / workflow）を 39 のアンチパターン指標で自動診断し、S〜E グレードと Quick Wins（修正提案）を出力する CLI ツール。
...
- `src/indicators/index.mjs` の INDICATORS 配列が 39 指標の SSOT（v0.5.0 で 36→39、IND-37/38/39 追加）。ドキュメントと乖離したら SSOT 側を正とする
