...
※ npm publish は近日予定。現時点では下記の GitHub clone を利用してください。
...
| まさお氏 `/review-harness` | 有料（¥29,800〜）、同コンセプト | 本ツールは独立派生・無料オープンソース版 |

## FAQ

**Q: まさお氏の `/review-harness` のコピーですか？**
A: いいえ。コンセプト（25指標 / S〜Eグレード / Quick Wins）は共通ですが、**有料記事は購読していません**。25指標の中身は Anthropic 公式ドキュメント、OpenSSF、IEEE 等の一般ベストプラクティスから独自に策定しました。
...
**Q: 指標を追加・カスタマイズできますか？**
A: v0.1.0 はビルトイン 25 指標のみ。ユーザー定義指標は将来検討。publisher allowlist 等の閾値は `~/.neko-harness-doctor/config.json` で拡張可能。
...
- **まさお氏** (@masa_wunder) の `/review-harness` 記事にコンセプト面で着想を得ました（有料記事購読なし、指標は独自策定）
...
- [ ] **v0.2.0** — `~/.neko-harness-doctor/config.json` 拡張、CI テスト整備、ユーザー定義指標
- [ ] **v0.3.0** — MCPツール説明文の実測（起動時動的取得）
